...
async def validate_authorization(bot: Bot, token: str, disable_static_tokens=False):
    ...
    if not disable_static_tokens:
        ...
        if token == static_token:
            return True
    token_obj = await bot.web_tokens.db.find_one({"token": token})
    if token_obj:
...
class APIRoutes:
    ...
    async def _get_web_token(
        ...
        has_token = await self.bot.web_tokens.find_by_id(request["user_id"])
        if has_token:
            if not int(datetime.datetime.now().timestamp()) > has_token["expires_at"]:
                return has_token
        # # # print(request)
        ...
        await self.bot.web_tokens.upsert(object)
